/**
 * JavaScript sẽ được inject vào trang web để thực hiện Object Spy.
 * Khi user hover → highlight phần tử. Ctrl+Click → capture và gửi về backend.
 * 
 * Hàm này trả về string JS để dùng trong driver.executeScript()
 */
...
(function() {
  ...
  function collectElementData(el) {
    ...
    // Sinh locators với thứ tự ưu tiên
    const locators = [];
    let priority = 1;

    if (id) {
      locators.push({ type: 'id', value: id, priority: priority++, status: 'active' });
    }
    if (dataTestId) {
      locators.push({ type: 'data-testid', value: dataTestId, priority: priority++, status: 'active' });
    }
    if (name) {
      locators.push({ type: 'name', value: name, priority: priority++, status: 'active' });
    }
    ...
    if (className) {
      locators.push({ type: 'className', value: className, priority: priority++, status: 'active' });
    }

    // Tạo objectId tự động dựa trên tag + thuộc tính
    let autoId = tagName;
    const prefixMap = {
  ...
  function onClick(e) {
    if (!e.ctrlKey) return; // Chỉ capture khi giữ Ctrl
